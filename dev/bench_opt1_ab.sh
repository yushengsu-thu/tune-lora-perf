#!/usr/bin/env bash
# opt1 A/B bench: LoRA cell with SGLANG_OPT_LORA_FUSED_MERGED_ALIGN off vs on, same warm pod.
# off = current LORA_ENVS (unfused align fallback); on = + the flag (fused single-launch align,
# now incl. shared_outer per the opt1 change). Captures prefill/decode/e2e per bs + coherence.
#   Usage: bash dev/bench_opt1_ab.sh <model>
. "$(dirname "$0")/common.sh" "${1:-}"
load_state

OUT="${DEV_DIR}/results/${MODEL}/opt1-ab"
echo "== [opt1-ab] $MODEL  bs=${BENCH_BS} in=${BENCH_IN} out=${BENCH_OUT}  -> ${OUT}"
LA="--lora-name ${LORA_NAME}"
BASE_LORA_ENVS="$LORA_ENVS"

for VAR in off on; do
  # NOTE: SGLANG_OPT_LORA_FUSED_MERGED_ALIGN defaults True, so "off" MUST set =0 explicitly
  # (an unset var = fused-on = no real baseline). on=1 (== default) takes the fused path.
  if [ "$VAR" = on ]; then LORA_ENVS="$BASE_LORA_ENVS SGLANG_OPT_LORA_FUSED_MERGED_ALIGN=1"
  else LORA_ENVS="$BASE_LORA_ENVS SGLANG_OPT_LORA_FUSED_MERGED_ALIGN=0"; fi
