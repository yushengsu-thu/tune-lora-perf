...
load_state
OUT="${DEV_DIR}/results/${MODEL}/opt1-ab/profile"
P_BS=16; P_START=6; P_STEPS=12; P_OUTLEN=48
echo "== [opt1-prof] graph-OFF bs${P_BS} start${P_START} steps${P_STEPS} -> ${OUT}"
...
kill_all
echo "== [opt1-prof] done -> ${OUT}/{off,on}/bs${P_BS}-TP-0.trace.json.gz"
